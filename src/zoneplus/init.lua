...
local CollectiveWorldModel = require(zoneControllerModule.CollectiveWorldModel)
local ZoneController = require(zoneControllerModule)

do
	local ZonePlusReference = require(script.ZonePlusReference)
	local referenceLocation = if RunService:IsClient() then "Client" else "Server"

	local referenceObject = ZonePlusReference.getObject()
	local referencePresent = if referenceObject
		then referenceObject:FindFirstChild(referenceLocation) ~= nil
		else false
...
-- CONSTRUCTORS
function ZonePlus.new(container)
	local self = {}
	setmetatable(self, ZonePlus)
	...
	for _, triggerType in pairs(triggerTypes) do
		...
		for i, triggerEvent in pairs(triggerEvents) do
			...
			-- so that we can act accordingly (i.e. begin or end a checker loop)
	...
	ZonePlus.touchedConnectionActions = {}
	for _, triggerType in pairs(triggerTypes) do
		...
		if correspondingMethod then
			...
			ZonePlus.touchedConnectionActions[triggerType] = function(touchedItem)
...
function ZonePlus:getContainer()
	return self.container
end

function ZonePlus.fromRegion(cframe, size)
	...
	local function createCube(cubeCFrame, cubeSize)
		...
		then
			...
			createCube(
				cubeCFrame * CFrame.new(-quarterSize.X, -quarterSize.Y, -quarterSize.Z),
				halfSize
			)
			createCube(
				cubeCFrame * CFrame.new(-quarterSize.X, -quarterSize.Y, quarterSize.Z),
...
function ZonePlus.is(self: any): boolean
	return getmetatable(self) == ZonePlus
end

-- PRIVATE METHODS
function ZonePlus:_calculateRegion(tableOfParts, dontRound)
...
function ZonePlus:_displayBounds()
...
function ZonePlus:_updateOccupants(trackerName, newOccupants)
...
function ZonePlus:_formTouchedConnection(triggerType)
...
function ZonePlus:_updateTouchedConnection(triggerType)
	local touchedJanitorName = "_touchedJanitor" .. triggerType
	local touchedJanitor = self[touchedJanitorName]
	...
	for _, basePart in pairs(self.zoneParts) do
		touchedJanitor:add(
			basePart.Touched:Connect(self.touchedConnectionActions[triggerType], self),
			"Disconnect"
		)
	end
end

function ZonePlus:_updateTouchedConnections()
...
function ZonePlus:_disconnectTouchedConnection(triggerType)
	local touchedJanitorName = "_touchedJanitor" .. triggerType
	local touchedJanitor = self[touchedJanitorName]
	if touchedJanitor then
		touchedJanitor:cleanup()
...
function ZonePlus:_partTouchedZone(part)
		...
		heartbeat:Connect(function()
			...
			if clockTime >= nextCheck then
				...
				if not verifiedEntrance then
					...
					elseif
						(part.Position - enterPosition).Magnitude > 1.5
...
function ZonePlus:_getRegionConstructor(part, overlapParams)
	local success, shapeName = pcall(function()
...
-- PUBLIC METHODS
function ZonePlus:findLocalPlayer()
...
function ZonePlus:_find(trackerName, item)
	ZoneController.updateDetection(self)
	local tracker = ZoneController.trackers[trackerName]
	local touchingZones =
		ZoneController.getTouchingZones(item, false, self._currentEnterDetection, tracker)
...
function ZonePlus:findPlayer(player)
...
function ZonePlus:findItem(item)
	return self:_find("item", item)
end

function ZonePlus:findPart(part)
...
function ZonePlus:getCheckerPart()
...
function ZonePlus:findPoint(positionOrCFrame)
	...
	local methodName, args =
		self:_getRegionConstructor(checkerPart, self.overlapParams.zonePartsWhitelist)
	local touchingZoneParts = self.worldModel[methodName](self.worldModel, unpack(args))
...
function ZonePlus:_getAll(trackerName)
	ZoneController.updateDetection(self)
	local itemsArray = {}
...
function ZonePlus:getPlayers()
	return self:_getAll("player")
end

function ZonePlus:getItems()
	return self:_getAll("item")
end

function ZonePlus:getParts()
...
function ZonePlus:getRandomPoint()
...
function ZonePlus:setAccuracy(enumIdOrName)
...
function ZonePlus:setDetection(enumIdOrName)
	local enumId = tonumber(enumIdOrName)
	if not enumId then
...
function ZonePlus:trackItem(instance)
...
function ZonePlus:untrackItem(instance)
...
function ZonePlus:bindToGroup(settingsGroupName)
...
function ZonePlus:unbindFromGroup()
...
function ZonePlus:relocate()
	...
	self.relocationContainer =
		self.janitor:add(relocationContainer, "Destroy", "RelocationContainer")
	relocationContainer.Parent = worldModel
end

function ZonePlus:_onItemCallback(eventName, desiredValue, instance, callbackFunction)
...
function ZonePlus:onItemEnter(...)
	self:_onItemCallback("itemEntered", true, ...)
end

function ZonePlus:onItemExit(...)
	self:_onItemCallback("itemExited", false, ...)
end

function ZonePlus:destroy()
	self:unbindFromGroup()
	self.janitor:destroy()
end
ZonePlus.Destroy = ZonePlus.destroy
