...
local heartbeatActions = {
	...
	["localPlayer"] = function(recommendedDetection)
		...
		local touchingZones =
			ZoneController.getTouchingZones(character, true, recommendedDetection, trackers.player)
	...
	heartbeatConnection = heartbeat:Connect(function()
		...
		if clockTime >= nextCheck then
			...
			for zone, newOccupants in pairs(zonesAndOccupants) do
				local settingsGroup = (
					zone.settingsGroupName and ZoneController.getGroup(zone.settingsGroupName)
				)
	...
	if totalRemainingBoundParts > 0 then
		...
		for _, bodyPart in pairs(bodyPartsToCheck) do
			...
			if
				...
				or (itemIsCharacter and Tracker.bodyPartsToIgnore[bodyPart.Name])
			then
				continue
			end
...
local workspaceContainerName =
	string.format("ZonePlus%sContainer", (runService:IsClient() and "Client") or "Server")
